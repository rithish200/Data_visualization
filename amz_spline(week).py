...
data["Week"] = data["reviews.date"].dt.strftime("Y: %Y W: %U")
week_avg = data.groupby(["Week"]).mean(["reviews.rating"])
...
chart_type = """
{
    ...
    tooltip: {
        ...
        pointFormat: ' Average of {point.y} Stars on this week'
    },
    plotOptions: {
        spline: {
        marker: {
...
def app():
    ...
    hc.options.series[0].data = list(week_avg["reviews.rating"])
    return wp
